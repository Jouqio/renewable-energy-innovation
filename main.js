...
(function initCursor() {
  ...
  document
    .querySelectorAll(
      "a, button, .etab-btn, .crisis-card, .tech-card, .fact-card, .impact-item, .cta-action",
    )
    .forEach((el) => {
...
(function initSound() {
  ...
  function startAll() {
    ...
    brownF.connect(brownG2);
    brownG2.connect(out);
    brown.start();
    allStarted.push(brown);
...
(function initReveal() {
  ...
  document
    .querySelectorAll(".crisis-visual")
    .forEach((el) => barIO.observe(el));
  ...
  document
    .querySelectorAll(".fact-card__number")
...
function animateCounter(el, target) {
  ...
  function step(now) {
    ...
    el.textContent = Math.round(easeOutQuart(p) * target).toLocaleString(
      "id-ID",
    );
...
(function initTabs() {
  ...
  document.querySelectorAll(".etab-btn").forEach((btn) => {
    function activateTab(e) {
      e.stopPropagation();
      document
        .querySelectorAll(".etab-btn")
        .forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      document
        .querySelectorAll(".energy-tab-content")
      ...
      if (t) {
        ...
        requestAnimationFrame(() => {
          t.style.animation = "tabFadeIn 0.45s ease forwards";
        });
      }
    }
    // Desktop click
...
(function initTilt() {
  ...
  if (window.matchMedia("(pointer: coarse)").matches) return;
  document
    .querySelectorAll(".tech-card, .fact-card, .crisis-card")
    .forEach((card) => {
      let raf;
